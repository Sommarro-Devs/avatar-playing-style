...
__all__ = ['PlayMakerRadar']


class PlayMakerRadar:
...
max_range=[10, 10, 10])
    ...
    def draw_circles(self, ring_width_scaler = 0.1, ax=None, inner=True, **kwargs):
...
max_range=[10, 10, 10])
        ...
        for idx, radius in enumerate(ax_circles):
            if (idx == 0) and first_center:
                ...
                width = self.ring_width*ring_width_scaler
...
kwargs_compare={'facecolor': '#d80499', 'alpha': 0.6})
    ...
    def draw_param_labels(self, active_params = [], ax=None, wrap=13, offset=1, kwargs_non_active_text=None, kwargs_active_text=None):
        ...
        **kwargs_non_active_text : All other keyword arguments are passed on to matplotlib.axes.Axes.text.
                                For the non acitve labels.
        **kwargs_active_text : All other keyword arguments are passed on to matplotlib.axes.Axes.text.
                                For the acitve labels.                        
        ...
        for idx, label in enumerate(self.params):          
            if label in active_params:
                if wrap is not None:
                    label = '\n'.join(textwrap.wrap(label, wrap, break_long_words=False))
                    
                
                text = ax.text(param_xs[idx], param_ys[idx], label,
                               rotation=self.rotation_degrees[idx], ha='center', va='center', **kwargs_active_text)
                label_list.append(text)
            else:
                if wrap is not None:
                    label = '\n'.join(textwrap.wrap(label, wrap, break_long_words=False))
                ...
                text = ax.text(param_xs[idx], param_ys[idx], label,
                               rotation=self.rotation_degrees[idx], ha='center', va='center', **kwargs_non_active_text)
                label_list.append(text)
        return label_list